...
class PlaceMapPage extends StatefulWidget {
  const PlaceMapPage({
    ...
    this.initialBudget = '',
    this.autoStartRoute = false,
    this.autoSelectTopResult = false,
  ...
  final bool autoStartRoute;
  final bool autoSelectTopResult;
...
class _PlaceMapPageState extends State<PlaceMapPage> {
  ...
  LatLng? pendingExternalFocus;
  bool externalLocationPresented = false;
      ...
      WidgetsBinding.instance.addPostFrameCallback((_) {
        if (mounted) {
          unawaited(
            runSearch(
              selectTopResult:
                  widget.autoStartRoute || widget.autoSelectTopResult,
              openRouteForTopResult: widget.autoStartRoute,
            ),
          );
        }
      });
  ...
  List<String> get effectivePreferences => widget.initialPreferences.isNotEmpty
  ...
  Future<void> initializeCurrentLocation() async {
    ...
    if (!externalLocationPresented) await moveMapTo(value, zoom: 16);
  }
  ...
  Future<void> runSearch({
    bool selectTopResult = false,
    bool openRouteForTopResult = false,
  }) async {
    ...
    try {
      final baseQuery = search.text.trim();
      final preferenceQuery =
          <String>[...effectivePreferences, widget.initialBudget]
              .map((value) => value.trim())
              .where((value) => value.isNotEmpty)
              .join(' ');
      final verifiedQuery = openRouteForTopResult && preferenceQuery.isNotEmpty
          ? '$baseQuery $preferenceQuery'
          : baseQuery;
      final d = await post('/api/places/search', {'query': verifiedQuery});
      final places = List<Map<String, dynamic>>.from(d['places'] ?? []);
      if (mounted) {
        setState(() {
          results = places;
      ...
      if (selectTopResult && places.isNotEmpty && mounted) {
        final routePlaces = places
            .where((place) => place['location'] is Map)
            .toList(growable: false);
        if (openRouteForTopResult && widget.isAiJourney) {
          await _openAiPlaceChoices(routePlaces);
          return;
        }
        // A city/state result commonly has no Google review text, but it is
        // still a valid route destination. Do not route automatic journeys
        // through the recommendation-analysis endpoint, which rejects such
        // area results before DirectionsSetupPage can open.
        final routePlace = places.cast<Map<String, dynamic>>().firstWhere(
          (place) => place['location'] is Map,
          orElse: () => const <String, dynamic>{},
        );
        if (routePlace.isNotEmpty) {
          _showSearchResult(routePlace, openRoute: openRouteForTopResult);
        }
      }
    } catch (e) {
      fail(e);
  ...
  Future<void> _openAiPlaceChoices(List<Map<String, dynamic>> places) async {
    if (places.isEmpty) {
      throw Exception('No verified route destinations were found.');
    }
    final current = await position();
    if (!mounted || current == null) {
      throw Exception('Location permission is required to compare routes.');
    }
    await Navigator.push(
      context,
      MaterialPageRoute<void>(
        builder: (_) => AiPlaceChoicePage(
          backend: backend,
          start: RouteLocation(
            name: 'Your location',
  ...
  void _showSearchResult(
    ...
    final target = LatLng(latitude.toDouble(), longitude.toDouble());
    setState(() {
      // Automatic attachment handoff owns this result. Hide autocomplete and
      // present the verified place directly on the map.
      results = [];
      externalLocationPresented = true;
      pendingExternalFocus = target;
      ...
      markers = {
        Marker(
          markerId: MarkerId(place['id']?.toString() ?? search.text.trim()),
          position: target,
        ),
      };
      message = null;
    ...
    if (openRoute) directions(place, autoStart: true);
  }

  Future<void> selectPlace(String id, {bool openRoute = false}) async {
    setState(() {
      loading = true;
      results = [];
      showCarousel = false;
    ...
    try {
      ...
      if (openRoute && mounted) {
        directions(p, autoStart: true);
      }
    } catch (e) {
      fail(e);
    } finally {
  ...
  void directions(Map p, {bool autoStart = false}) {
    ...
    body: Stack(
      children: [
        GoogleMap(
          ...
          onMapCreated: (c) {
            controller = c;
            final externalFocus = pendingExternalFocus;
            if (externalFocus != null) {
              unawaited(
                c.animateCamera(CameraUpdate.newLatLngZoom(externalFocus, 15)),
              );
