...
class _ActiveItineraryMapScreenState extends State<ActiveItineraryMapScreen> {
  ...
  void initState() {
    ...
    _memberSubscription = _liveLocationService.watchLocations().listen((
      members,
    ) {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body: SafeArea(
        ...
        child: Stack(
          children: [
            Positioned.fill(top: 88, child: _buildMap(currentIndex)),
            Positioned(
              left: 0,
              right: 0,
              top: 0,
              child: _NavigationHeader(
                instruction: isFinalStop
                    ? 'Continue for 300 m'
            ...
            for (final entry in members.indexed)
              ListTile(
                ...
                leading: CircleAvatar(
                  ...
                  child: Text(
                    _initials(entry.$2.displayName),
                    style: const TextStyle(color: Colors.white, fontSize: 11),
                  ),
                ),
                title: Text(
                  entry.$2.isCurrentUser
